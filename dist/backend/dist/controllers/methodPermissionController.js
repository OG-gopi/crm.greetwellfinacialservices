...
exports.getAllMethodPermissions = getAllMethodPermissions;
exports.createMethodPermission = createMethodPermission;
exports.updateMethodPermission = updateMethodPermission;
exports.deleteMethodPermission = deleteMethodPermission;
exports.toggleMethodPermissionStatus = toggleMethodPermissionStatus;
...
async function getAllMethodPermissions(req, res) {
    try {
...
async function createMethodPermission(req, res) {
    try {
        ...
        const newDef = await prisma_1.prisma.methodPermissionDef.create({
            data: {
                methodName,
                endpoint: endpoint || '',
        ...
        for (const r of ALL_ROLES) {
            ...
            await prisma_1.prisma.roleMethodPermission.create({
                data: {
                    role: r,
        ...
        const fullDef = await prisma_1.prisma.methodPermissionDef.findUnique({
            where: { id: newDef.id },
...
async function updateMethodPermission(req, res) {
    try {
        const { id } = req.params;
        const { methodName, endpoint, httpMethod, permissionType, isActive, description, rolePermissions } = req.body;
        const existing = await prisma_1.prisma.methodPermissionDef.findUnique({ where: { id } });
        if (!existing) {
            return res.status(404).json({ success: false, message: 'Method permission not found.' });
        }
        const updated = await prisma_1.prisma.methodPermissionDef.update({
            where: { id },
            data: {
        ...
        if (rolePermissions && typeof rolePermissions === 'object') {
            for (const r of ALL_ROLES) {
                if (rolePermissions[r] !== undefined) {
                    await prisma_1.prisma.roleMethodPermission.upsert({
...
async function deleteMethodPermission(req, res) {
    try {
        const { id } = req.params;
        const existing = await prisma_1.prisma.methodPermissionDef.findUnique({ where: { id } });
        if (!existing) {
        ...
        await prisma_1.prisma.methodPermissionDef.delete({ where: { id } });
        await (0, auditService_1.createAuditLog)({
            userId: req.user?.id,
            userRole: req.user?.role,
...
async function toggleMethodPermissionStatus(req, res) {
    try {
        const { id } = req.params;
