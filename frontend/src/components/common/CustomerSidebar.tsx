      ...
      >
        <div className="flex flex-col h-full overflow-hidden">
          ...
          <div className="mx-3 my-2 p-2.5 rounded-xl bg-white text-[#0E1A2B] text-xs font-semibold flex items-center gap-2.5 shadow-2xs">
            <UserCheck className="w-4 h-4 text-[#1E4FD6] shrink-0" />
            {isExpanded && <span className="truncate font-bold">Customer Portal</span>}
          </div>
          ...
          <nav className="flex-1 px-3 py-2 overflow-y-auto space-y-1.5 custom-scrollbar">
            ...
            <div>
              <button
                ...
                title={!isExpanded ? 'My Applications' : undefined}
              ...
              >
                ...
                {isExpanded && (
                  <>
                    <span className="truncate text-left flex-1">My Applications</span>
                    <ChevronDown
            ...
            <NavLink
              ...
              onClick={handleItemClick}
              title={!isExpanded ? 'My Documents' : undefined}
              className={({ isActive }) => getNavItemClasses(isActive)}
            >
              <FolderOpen className="w-[19px] h-[19px] shrink-0" />
              {isExpanded && <span className="truncate">My Documents</span>}
            </NavLink>
            ...
            <NavLink
              ...
              onClick={handleItemClick}
              title={!isExpanded ? 'Help & Enquiries' : undefined}
              className={({ isActive }) => getNavItemClasses(isActive)}
            >
              <HelpCircle className="w-[19px] h-[19px] shrink-0" />
              {isExpanded && <span className="truncate">Help & Enquiries</span>}
            </NavLink>
            ...
            <NavLink
              to="/customer/updates"
              onClick={handleItemClick}
              title={!isExpanded ? 'Platform Updates' : undefined}
              className={({ isActive }) => getNavItemClasses(isActive)}
            >
              <Sparkles className="w-[19px] h-[19px] shrink-0" />
              {isExpanded && <span className="truncate">Platform Updates</span>}
            </NavLink>
