      ...
      >
        <div className="flex flex-col h-full overflow-hidden">
          ...
          <div className="mx-3 my-2 p-2.5 rounded-xl bg-white text-[#0E1A2B] text-xs font-semibold flex items-center gap-2.5 shadow-2xs">
            ...
            {isExpanded && <span className="truncate font-bold">Insurance Agent Desk</span>}
          </div>
          ...
          <nav className="flex-1 px-3 py-2 overflow-y-auto space-y-1.5 custom-scrollbar">
            ...
            <div>
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13.5px] font-medium transition-all cursor-pointer ${
                  location.pathname.startsWith('/insurance-agent/customers')
                    ? 'bg-[#1E4FD6] text-white font-semibold shadow-md shadow-blue-600/30'
            ...
            <div>
              <button
                ...
                title={!isExpanded ? 'Insurance Policies' : undefined}
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13.5px] font-medium transition-all cursor-pointer ${
                  location.pathname.startsWith('/insurance-agent/applications') || location.pathname.startsWith('/insurance-agent/create-application')
                    ? 'bg-[#1E4FD6] text-white font-semibold shadow-md shadow-blue-600/30'
              ...
              >
                ...
                {isExpanded && (
                  <>
                    <span className="truncate text-left flex-1">Insurance Policies</span>
                    <ChevronDown
                      className={`w-3.5 h-3.5 shrink-0 transition-transform ${
              ...
              {isExpanded && openSubMenus['applications'] && (
                <div className="pl-8 pr-1 py-1 space-y-1">
                  ...
                  <NavLink
                    ...
                    onClick={handleItemClick}
                    className="block py-1.5 px-2.5 rounded-lg text-xs font-bold text-purple-300 hover:text-white hover:bg-[#16273D] flex items-center gap-1 mt-1 pt-1.5 border-t border-[#1E2F45]"
                  >
            ...
            <div>
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13.5px] font-medium transition-all cursor-pointer ${
                  location.pathname.startsWith('/insurance-agent/documents')
                    ? 'bg-[#1E4FD6] text-white font-semibold shadow-md shadow-blue-600/30'
            ...
            <NavLink
              ...
              onClick={handleItemClick}
              title={!isExpanded ? 'Policy Reports' : undefined}
              className={({ isActive }) => getNavItemClasses(isActive)}
            >
              <BarChart3 className="w-[19px] h-[19px] shrink-0" />
              {isExpanded && <span className="truncate">Policy Reports</span>}
            </NavLink>
            ...
            <NavLink
              ...
              onClick={handleItemClick}
              title={!isExpanded ? 'Support & Claims' : undefined}
              className={({ isActive }) => getNavItemClasses(isActive)}
            >
              <HelpCircle className="w-[19px] h-[19px] shrink-0" />
              {isExpanded && <span className="truncate">Support &amp; Claims</span>}
            </NavLink>
          </nav>
