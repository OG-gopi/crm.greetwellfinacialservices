...
class EmailService {
    ...
    async sendMail(options) {
        const { to, recipientName, subject, html, emailType, emailCategory = 'TRANSACTIONAL', relatedEntity, relatedEntityId, applicationId, actionUrl, } = options;
        if (!to || !to.trim()) {
            console.warn('⚠️ sendMail invoked with empty recipient email address.');
            return false;
        }
        const cleanTo = to.trim().toLowerCase();
        const senderEmail = config_1.CONFIG.SMTP.USER || 'greetwell.notify@gmail.com';
        // 1. Create PENDING delivery log record in DB
        let logRecord = null;
        try {
            logRecord = await prisma_1.prisma.emailDeliveryLog.create({
                data: {
        ...
        const smtpPass = process.env.GMAIL_APP_PASSWORD || process.env.SMTP_PASSWORD || config_1.CONFIG.SMTP.PASS;
        ...
        if (!smtpPass && process.env.NODE_ENV === 'production') {
            ...
            if (actionUrl)
                console.log(`CTA Action URL: ${actionUrl}`);
            console.log(`==================================================\n`);
            isSuccess = true;
        }
        ...
        if (logRecord) {
            try {
                await prisma_1.prisma.emailDeliveryLog.update({
     ...
     */
    async retryFailedEmail(logId) {
        try {
            const log = await prisma_1.prisma.emailDeliveryLog.findUnique({
                where: { id: logId },
            });
            if (!log) {
            ...
            try {
                await this.transporter.sendMail({
                    ...
                    subject: log.subject,
                    html: this.renderGenericBody(log.subject, `This is a retried notification email regarding your portal account.`, log.actionUrl || `${config_1.CONFIG.APP_URL}/login`),
                });
                await prisma_1.prisma.emailDeliveryLog.update({
                    where: { id: logId },
                    data: {
              ...
              <table border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 520px; background-color: #ffffff; border-radius: 10px; border: 1px solid #e2e8f0; overflow: hidden; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.04);">
                ...
                <tr>
                  <td style="background-color: #0b1320; padding: 12px 16px; text-align: center; border-bottom: 2px solid #C99A3E;">
                    <img src="${config_1.CONFIG.FRONTEND_URL}/logo.png" alt="Greetwell Financial Services" style="max-height: 48px; width: auto; display: block; margin: 0 auto;" />
    ...
    renderGenericBody(title, message, url) {
    ...
    async sendAgentRegistrationVerification(options) {
        ...
        const verifyUrl = `${config_1.CONFIG.APP_URL}/invite/${token}`;
    ...
    async sendForgotPassword(options) {
        ...
        const resetUrl = `${config_1.CONFIG.APP_URL}/forgot-password?token=${token}`;
    ...
    async sendPasswordChanged(options) {
        ...
        const html = this.renderBrandTemplate({
            ...
            ctaButton: { label: 'Log In to Portal →', url: `${config_1.CONFIG.APP_URL}/login` },
        });
        return this.sendMail({
            to: email,
            recipientName,
            ...
            emailCategory: 'AUTH',
            relatedEntity: 'USER',
            actionUrl: `${config_1.CONFIG.APP_URL}/login`,
        });
    }
    // =========================================================================
    // 7. INVITATION EMAILS
    ...
    async sendInvitationAcceptedNotification(options) {
        ...
        if (inviterEmail && inviterEmail.trim()) {
            ...
            const inviterHtml = this.renderBrandTemplate({
                ...
                ctaButton: { label: 'View Users in Admin Portal →', url: `${config_1.CONFIG.APP_URL}/superadmin/users` },
            });
            await this.sendMail({
                to: inviterEmail,
                recipientName: inviterName || 'Admin',
                ...
                emailCategory: 'TRANSACTIONAL',
                actionUrl: `${config_1.CONFIG.APP_URL}/superadmin/users`,
            });
        }
        return true;
    }
    ...
    async sendAgentAssignedNotification(options) {
        ...
        const custHtml = this.renderBrandTemplate({
            ...
            ctaButton: { label: 'View Customer Dashboard →', url: `${config_1.CONFIG.APP_URL}/customer/dashboard` },
        });
        await this.sendMail({
            to: customerEmail,
            recipientName: customerName,
            ...
            emailCategory: 'TRANSACTIONAL',
            actionUrl: `${config_1.CONFIG.APP_URL}/customer/dashboard`,
        });
        // Notice to Agent
        ...
        const agentHtml = this.renderBrandTemplate({
            ...
            ctaButton: { label: 'View Customer Profile →', url: `${config_1.CONFIG.APP_URL}/agent/customers` },
        });
        await this.sendMail({
            to: agentEmail,
            recipientName: agentName,
            ...
            emailCategory: 'TRANSACTIONAL',
            actionUrl: `${config_1.CONFIG.APP_URL}/agent/customers`,
        });
        return true;
    }
    async sendCustomerReassignedNotification(options) {
        ...
        const html = this.renderBrandTemplate({
            ...
            ctaButton: { label: 'Go to Customer Portal →', url: `${config_1.CONFIG.APP_URL}/customer/dashboard` },
        });
        return this.sendMail({
            to: customerEmail,
            recipientName: customerName,
    ...
    async sendApplicationCreatedNotification(options) {
        ...
        const viewUrl = `${config_1.CONFIG.APP_URL}/customer/applications`;
        ...
        if (agentEmail && agentEmail.trim()) {
            ...
            const agentHtml = this.renderBrandTemplate({
                ...
                ctaButton: { label: 'Review Application in Agent Desk →', url: `${config_1.CONFIG.APP_URL}/loan-agent/applications` },
            });
            await this.sendMail({
                to: agentEmail,
                recipientName: agentName,
                ...
                relatedEntityId: applicationId,
                applicationId,
                actionUrl: `${config_1.CONFIG.APP_URL}/loan-agent/applications`,
            });
        }
        return true;
    }
    async sendApplicationStatusUpdatedEmail(options) {
        ...
        const actionUrl = isCustomer
            ? `${config_1.CONFIG.APP_URL}/customer/applications`
            : `${config_1.CONFIG.APP_URL}/superadmin/applications/all`;
        const formattedPrev = previousStatus.replace(/_/g, ' ');
        const formattedNew = newStatus.replace(/_/g, ' ');
        let statusColor = '#2563eb';
        if (['APPROVED', 'COMPLETED', 'VERIFIED'].includes(newStatus))
            statusColor = '#16a34a';
        if (['REJECTED', 'CANCELLED'].includes(newStatus))
            statusColor = '#dc2626';
        if (['INFORMATION_REQUIRED', 'DOCUMENTS_REQUIRED'].includes(newStatus))
            statusColor = '#d97706';
    ...
    async sendApplicationCommentNotification(options) {
        ...
        const actionUrl = isCustomer ? `${config_1.CONFIG.APP_URL}/customer/applications` : `${config_1.CONFIG.APP_URL}/loan-agent/applications`;
    ...
    async sendDocumentRequestedEmail(options) {
        ...
        const uploadUrl = `${config_1.CONFIG.APP_URL}/customer/documents`;
    ...
    async sendDocumentUploadedNotification(options) {
        ...
        const reviewUrl = `${config_1.CONFIG.APP_URL}/superadmin/documents`;
    ...
    async sendDocumentApprovedEmail(options) {
        ...
        const viewUrl = `${config_1.CONFIG.APP_URL}/customer/documents`;
    ...
    async sendEnquiryClosedNotification(options) {
        ...
        const html = this.renderBrandTemplate({
            ...
            ctaButton: { label: 'Go to Support Portal →', url: `${config_1.CONFIG.APP_URL}/customer/enquiries` },
        });
        return this.sendMail({
            to: email,
            recipientName: userName,
            ...
            relatedEntity: 'ENQUIRY',
            relatedEntityId: enquiryId,
            actionUrl: `${config_1.CONFIG.APP_URL}/customer/enquiries`,
        });
    }
    // =========================================================================
    // 12. ADMIN ALERTS
    ...
    async sendAdminAlert(options) {
        ...
        const targetUrl = actionUrl || `${config_1.CONFIG.APP_URL}/superadmin/dashboard`;
