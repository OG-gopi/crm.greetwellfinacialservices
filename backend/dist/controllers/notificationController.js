...
async function getNotifications(req, res) {
    try {
        ...
        const { module, source, readStatus, actionStatus, search, page = '1', limit = '20', startDate, endDate, } = req.query;
        ...
        if (userRole === 'SUPER_ADMIN') {
            ...
            const enabledServices = (0, validation_1.safeParseJsonArray)(customerRecord?.serviceTypes).map((s) => s.toUpperCase());
            ...
            const customerApps = await prisma_1.prisma.application.findMany({
        ...
        const AND_conditions = [roleWhereClause];
        if (module && module !== 'ALL') {
            AND_conditions.push({ module: module.toUpperCase() });
        }
        if (source && source !== 'ALL') {
            AND_conditions.push({ source: source.toUpperCase() });
        }
        ...
        if (actionStatus && actionStatus !== 'ALL') {
            AND_conditions.push({ actionStatus: actionStatus.toUpperCase() });
        }
        ...
        if (startDate) {
            AND_conditions.push({ createdAt: { gte: new Date(startDate) } });
        }
        if (endDate) {
            AND_conditions.push({ createdAt: { lte: new Date(endDate) } });
        }
        ...
        const [notifications, totalCount, unreadCount, actionRequiredCount, actionTakenCount, notRequiredCount] = await Promise.all([
            ...
            prisma_1.prisma.notification.count({ where: finalWhere }),
...
async function updateNotificationStatus(req, res) {
    try {
        ...
        if (actionStatus)
            updateData.actionStatus = actionStatus;
