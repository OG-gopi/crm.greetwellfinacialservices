...
exports.replyToApplicationRequest = replyToApplicationRequest;
...
async function getApplications(req, res) {
    try {
        ...
        if (user.role === 'CUSTOMER') {
            ...
            if (type) {
                if (!enabledTypes.includes(type.toUpperCase())) {
...
async function getApplicationById(req, res) {
    try {
        ...
        if ((user.role === 'LOAN_AGENT' && (application.type !== 'LOAN' || application.assignedAgentId !== user.id)) ||
...
async function createApplication(req, res) {
    try {
        ...
        if (user.role === 'CUSTOMER') {
            const customerRecord = await prisma_1.prisma.user.findUnique({
                where: { id: user.id },
                select: { serviceTypes: true },
            });
            const customerServices = (0, validation_1.safeParseJsonArray)(customerRecord?.serviceTypes).map((s) => s.toUpperCase());
            const categoryMap = {
                LOAN: ['LOAN', 'LOANS'],
                INSURANCE: ['INSURANCE'],
        ...
        if (mobileToValidate) {
            const mobCheck = (0, validation_1.validateIndianMobile)(mobileToValidate);
            if (!mobCheck.isValid) {
        ...
        if (customerPhone && customerPhone !== 'N/A') {
            const mobCheck = (0, validation_1.validateIndianMobile)(customerPhone);
            if (mobCheck.isValid && mobCheck.cleanPhone) {
                await whatsappService_1.whatsAppService.notifyApplicationCreated(mobCheck.cleanPhone, `${newApp.customer?.firstName || 'Customer'}`, newApp.id, type, 'SUBMITTED');
            }
        }
...
async function assignApplication(req, res) {
    try {
        ...
        const application = await prisma_1.prisma.application.findUnique({ where: { id } });
        if (!application) {
            return res.status(404).json({ success: false, message: 'Application not found.' });
        }
...
async function updateApplicationStatus(req, res) {
    try {
        ...
        if (note) {
            await prisma_1.prisma.note.create({
                data: {
                    applicationId: id,
...
async function verifyApplicationData(req, res) {
    try {
        const { id } = req.params;
...
async function createApplicationRequest(req, res) {
    try {
        const { id } = req.params;
        ...
        const requirement = await prisma_1.prisma.applicationRequirement.create({
            data: {
                applicationId: id,
                title: title.trim(),
        ...
        if (application.customer?.email) {
            const customerName = `${application.customer.firstName} ${application.customer.lastName || ''}`.trim();
            await emailService_1.emailService.sendDocumentRequestedEmail({
        ...
        if (application.customer.phone) {
            const mobCheck = (0, validation_1.validateIndianMobile)(application.customer.phone);
            if (mobCheck.isValid && mobCheck.cleanPhone) {
                await whatsappService_1.whatsAppService.notifyDocumentRequest(mobCheck.cleanPhone, application.customer.firstName, id, title, description);
            }
        }
        await (0, auditService_1.createAuditLog)({
            userId: user.id,
...
async function replyToApplicationRequest(req, res) {
    try {
        const { requestId } = req.params;
        const { customerReply, replyDocUrl } = req.body;
        const user = req.user;
        const requirement = await prisma_1.prisma.applicationRequirement.findUnique({
            where: { id: requestId },
            include: {
                application: {
                    include: { customer: true, assignedAgent: true },
        ...
        const updatedRequirement = await prisma_1.prisma.applicationRequirement.update({
            where: { id: requestId },
            data: {
        ...
        if (replyDocUrl) {
            ...
            if (targetAgentEmail) {
                await emailService_1.emailService.sendDocumentUploadedNotification({
                ...
                await emailService_1.emailService.notifySuperAdmins({
        ...
        if (requirement.application.assignedAgentId) {
            ...
            if (requirement.application.assignedAgent?.phone) {
                const mobCheck = (0, validation_1.validateIndianMobile)(requirement.application.assignedAgent.phone);
                if (mobCheck.isValid && mobCheck.cleanPhone) {
                    await whatsappService_1.whatsAppService.notifyCustomerReply(mobCheck.cleanPhone, requirement.application.assignedAgent.firstName, requirement.applicationId, requirement.title);
                }
            }
        }
        await (0, auditService_1.createAuditLog)({
...
async function updateRequestStatus(req, res) {
    try {
        ...
        const requirement = await prisma_1.prisma.applicationRequirement.update({
            where: { id: requestId },
            data: { status },
        });
        return res.json({
