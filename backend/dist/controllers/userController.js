...
exports.createAgentInvitation = createAgentInvitation;
exports.inviteCustomer = inviteCustomer;
exports.updateUserStatus = updateUserStatus;
exports.editUser = editUser;
exports.resendAgentInvitation = resendAgentInvitation;
exports.getInvitations = getInvitations;
exports.updateCustomerServices = updateCustomerServices;
exports.requestServiceAccess = requestServiceAccess;
...
async function getUsers(req, res) {
    try {
        ...
        if (['LOAN_AGENT', 'INSURANCE_AGENT', 'INVESTMENT_AGENT'].includes(currentUser.role)) {
            const assignedApps = await prisma_1.prisma.application.findMany({
        ...
        const formattedUsers = users.map((u) => ({
            ...u,
            serviceTypes: (0, validation_1.safeParseJsonArray)(u.serviceTypes),
        }));
        return res.json({
            success: true,
            data: formattedUsers,
...
async function getUserById(req, res) {
    try {
        ...
        if (['LOAN_AGENT', 'INSURANCE_AGENT', 'INVESTMENT_AGENT'].includes(currentUser.role) && id !== currentUser.id) {
            const isAssignedCustomer = await prisma_1.prisma.application.findFirst({
        ...
        return res.json({
            ...
            data: {
                ...safeUser,
                serviceTypes: (0, validation_1.safeParseJsonArray)(safeUser.serviceTypes),
            },
        });
    }
    catch (err) {
...
async function createAgentInvitation(req, res) {
    try {
        ...
        const emailSent = await emailService_1.emailService.sendAgentInvitation({
            email: cleanEmail,
            role: agentType,
            token,
            firstName: firstName.trim(),
...
async function inviteCustomer(req, res) {
    try {
        ...
        const emailSent = await emailService_1.emailService.sendCustomerInvitation({
            email: cleanEmail,
            token,
            firstName: firstName.trim(),
            lastName: lastName.trim(),
        ...
        if (inviter.role !== 'SUPER_ADMIN') {
            await (0, notificationService_1.notifySuperAdmins)('CUSTOMER_INVITED', 'New Customer Invited', `Agent ${inviter.firstName} ${inviter.lastName} invited customer ${firstName} ${lastName} (${customerIdCode}) for ${serviceTypes.join(', ')}.`, {
                module: 'CUSTOMERS',
                source: 'AGENT',
                actionStatus: 'NONE',
                agentId: inviter.id,
...
async function editUser(req, res) {
    try {
        const { id } = req.params;
        const { firstName, lastName, phone, role, status, dob, education, hasExperience, previousCompany, previousJobRole, yearsOfExperience, previousJobStartDate, previousJobEndDate, aadhaarDocUrl, educationDocUrl, experienceDocUrl, otherDocUrl, } = req.body;
        const dataToUpdate = {};
        ...
        if (hasExperience !== undefined)
            dataToUpdate.hasExperience = Boolean(hasExperience);
        if (previousCompany !== undefined)
            dataToUpdate.previousCompany = previousCompany ? previousCompany.trim() : null;
        if (previousJobRole !== undefined)
            dataToUpdate.previousJobRole = previousJobRole ? previousJobRole.trim() : null;
        if (yearsOfExperience !== undefined)
            dataToUpdate.yearsOfExperience = yearsOfExperience ? String(yearsOfExperience) : null;
        if (previousJobStartDate !== undefined)
            dataToUpdate.previousJobStartDate = previousJobStartDate ? new Date(previousJobStartDate) : null;
        if (previousJobEndDate !== undefined)
            dataToUpdate.previousJobEndDate = previousJobEndDate ? new Date(previousJobEndDate) : null;
        if (aadhaarDocUrl !== undefined)
            dataToUpdate.aadhaarDocUrl = aadhaarDocUrl;
        if (educationDocUrl !== undefined)
            dataToUpdate.educationDocUrl = educationDocUrl;
        if (experienceDocUrl !== undefined)
            dataToUpdate.experienceDocUrl = experienceDocUrl;
        if (otherDocUrl !== undefined)
            dataToUpdate.otherDocUrl = otherDocUrl;
        const user = await prisma_1.prisma.user.update({
            where: { id },
            data: dataToUpdate,
        });
...
async function resendAgentInvitation(req, res) {
    try {
        const { id } = req.params;
        const invitation = await prisma_1.prisma.invitation.findUnique({ where: { id } });
        if (!invitation) {
...
async function getInvitations(req, res) {
    try {
        ...
        const formatted = invitations.map((inv) => ({
            ...inv,
            serviceTypes: (0, validation_1.safeParseJsonArray)(inv.serviceTypes),
        }));
...
async function updateCustomerServices(req, res) {
    try {
        ...
        const validServices = ['LOANS', 'INSURANCE', 'INVESTMENT'];
        const uppercaseServices = serviceTypes.map((s) => s.toUpperCase());
        const invalid = uppercaseServices.filter((s) => !validServices.includes(s));
        if (invalid.length > 0) {
...
async function requestServiceAccess(req, res) {
    try {
        const user = req.user;
        ...
        const currentUser = await prisma_1.prisma.user.findUnique({ where: { id: user.id } });
        if (!currentUser) {
            return res.status(404).json({ success: false, message: 'User account not found.' });
        }
        const existingServices = (0, validation_1.safeParseJsonArray)(currentUser.serviceTypes);
        ...
        await (0, notificationService_1.notifySuperAdmins)('SERVICE_ACCESS_REQUEST', 'New Service Access Request', `Customer ${user.firstName} ${user.lastName || ''} (${currentUser.customerIdCode || user.email}) requested access for service: ${serviceCode}.${reason ? ' Reason: ' + reason : ''}`, { module: 'USER', relatedEntityId: user.id });
        return res.json({
            success: true,
...
async function createUserAccount(req, res) {
    try {
        const { email, firstName, lastName, phone, password, role, status, serviceTypes, } = req.body;
        const adminUserId = req.user?.id;
        ...
        let customerIdCode = null;
        let agentIdCode = null;
        if (role === 'CUSTOMER') {
            customerIdCode = await (0, appId_1.generateCustomerId)();
        }
        else if (['LOAN_AGENT', 'INSURANCE_AGENT', 'INVESTMENT_AGENT'].includes(role)) {
            agentIdCode = await (0, appId_1.generateAgentId)();
        }
        // 5. Save User
...
async function createSuperAdminInvitation(req, res) {
    try {
        ...
        const pendingInvitation = await prisma_1.prisma.invitation.findFirst({
            where: {
                email: cleanEmail,
        ...
        const emailSent = await emailService_1.emailService.sendSuperAdminInvitation({
            email: cleanEmail,
            token,
            firstName: firstName.trim(),
            lastName: lastName ? lastName.trim() : undefined,
...
async function cancelInvitation(req, res) {
    try {
        ...
        await prisma_1.prisma.invitation.delete({ where: { id } });
        await (0, auditService_1.createAuditLog)({
            userId: adminUserId,
            userRole: req.user?.role,
